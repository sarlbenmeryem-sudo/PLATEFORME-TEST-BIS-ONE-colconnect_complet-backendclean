...
# ============================
# Patch: WAF custom rules production (anti-scan + optional docs allowlist)
# ID: CC_PATCH_WAF_CUSTOM_RULES_PROD_V1_20260301
# ============================

RG="rg-colconnect-prod-frc"
APPGW="agw-colconnect-prod"

# Optional: lock /api/docs to your IPs (empty = disabled)
...
echo ""
echo "== [1] Anti-scan path blocks =="
# Upsert by delete+create
...
az_retry az network application-gateway waf-policy custom-rule create \
  ...
  --match-conditions "[{
    ...
    \"operator\": \"Regex\",
    \"matchValues\": [\"(?i)(/wp-admin|/wp-login\\.php|/\\.env|/\\.git|/phpmyadmin|/pma|/cgi-bin|/vendor/phpunit|/actuator|/\\.well-known/assetlinks\\.json)\"] ,
...
echo ""
echo "== [2] Optional: restrict /api/docs & /openapi.json to allowlist IPs =="
...
if [[ "${#DOCS_ALLOWLIST_IPS[@]}" -gt 0 ]]; then
  ...
  # Rule: if RequestUri matches docs/openapi AND source IP NOT in allowlist => Block
  az_retry az network application-gateway waf-policy custom-rule create \
    -g "$RG" --policy-name "$policy_name" \
    ...
    --match-conditions "[
      {
        \"matchVariables\": [{\"variableName\":\"RequestUri\"}],
        \"operator\": \"Regex\",
        \"matchValues\": [\"(?i)^/api/docs$|(?i)^/openapi\\.json$\"],
        \"negationConditon\": false,
        \"transforms\": []
      },
      {
