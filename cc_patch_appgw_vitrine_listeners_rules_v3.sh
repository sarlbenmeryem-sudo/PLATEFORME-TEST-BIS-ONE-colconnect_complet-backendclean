...
# ============================
# Patch: AppGW listeners + rules for colconnect.fr (root) and www redirect
# ID: CC_PATCH_APPGW_VITRINE_LISTENERS_RULES_V3_20260301
# ============================
...
az network application-gateway address-pool show -g "$RG" --gateway-name "$APPGW" -n "$BP" -o none
az network application-gateway http-settings show -g "$RG" --gateway-name "$APPGW" -n "$BHS" -o none

echo "== [1] Frontend port 443 name =="
