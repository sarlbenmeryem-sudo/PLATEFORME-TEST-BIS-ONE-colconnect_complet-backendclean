...
# ============================
# Patch: Create URL path-map without --default-probe (CLI compat) + enforce HTTPS PathBasedRouting
# ID: CC_PATCH_APPGW_FRONT_VITRINE_ROUTING_V7_NO_DEFAULT_PROBE_20260301
# ============================
...
echo "== [CC_PATCH_APPGW_FRONT_VITRINE_ROUTING_V7_NO_DEFAULT_PROBE] Start =="
...
echo "== Create path map pm-front (no --default-probe; probe is in http-settings) =="
if az network application-gateway url-path-map show -g "$RG" --gateway-name "$APPGW" -n "$PATHMAP" >/dev/null 2>&1; then
  echo "ℹ️ Path map exists. Deleting to recreate cleanly: $PATHMAP"
  az network application-gateway url-path-map delete -g "$RG" --gateway-name "$APPGW" -n "$PATHMAP" -o none
fi
...
echo "== HTTPS -> PathBasedRouting (front default + /api redirect) =="
