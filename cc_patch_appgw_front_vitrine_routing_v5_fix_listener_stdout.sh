...
# ============================
# Patch: AppGW vitrine routing (fix ensure_listener stdout pollution)
# ID: CC_PATCH_APPGW_FRONT_VITRINE_ROUTING_V5_FIX_LISTENER_STDOUT_20260301
# ============================
...
echo "== [CC_PATCH_APPGW_FRONT_VITRINE_ROUTING_V5_FIX_LISTENER_STDOUT] Start =="
...
# ---- listeners (STDERR logs, STDOUT = ONLY listener name) ----
ensure_listener () {
  local want_name="$1"
  local port_name="$2"
  local host_name="$3"
  ...
  if [[ -n "${existing:-}" && "${existing}" != "None" ]]; then
    echo "Listener exists for $host_name:$port_name -> $existing (updating)" >&2
    if [[ -n "${ssl_cert:-}" ]]; then
  ...
  echo "Creating listener $want_name for $host_name:$port_name" >&2
  if [[ -n "${ssl_cert:-}" ]]; then
