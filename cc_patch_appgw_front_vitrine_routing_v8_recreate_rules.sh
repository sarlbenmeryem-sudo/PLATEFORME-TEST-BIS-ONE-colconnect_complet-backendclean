...
# ============================
# Patch: Front vitrine routing - recreate routing rules to avoid "multiple backend pools" on update
# ID: CC_PATCH_APPGW_FRONT_VITRINE_ROUTING_V8_RECREATE_RULES_20260301
# ============================
...
echo "== [CC_PATCH_APPGW_FRONT_VITRINE_ROUTING_V8_RECREATE_RULES] Start =="
...
echo "== Helper: delete rule bound to a listener (if any) =="
delete_rule_by_listener () {
  local listener="$1"
  local rname
  rname="$(az network application-gateway show -g "$RG" -n "$APPGW" \
    --query "requestRoutingRules[?contains(httpListener.id,'/httpListeners/$listener')].name | [0]" -o tsv)"
  if [[ -n "${rname:-}" && "${rname}" != "None" ]]; then
    echo "ℹ️ Deleting existing rule bound to $listener: $rname"
    az network application-gateway rule delete -g "$RG" --gateway-name "$APPGW" -n "$rname" -o none
  fi
}

echo "== Recreate HTTP redirect rules (delete-by-listener then create) =="
delete_rule_by_listener "$L80_ROOT"
...
delete_rule_by_listener "$L80_WWW"
az network application-gateway rule create \
...
echo "== Recreate HTTPS path-based rules (delete-by-listener then create) =="
delete_rule_by_listener "$L443_ROOT"
az network application-gateway rule create \
...
delete_rule_by_listener "$L443_WWW"
az network application-gateway rule create \
